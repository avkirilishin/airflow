...
class EdgeModifier(DependencyMixin):
    ...
    def set_upstream(
        ...
        for item in self._make_list(other):
            if isinstance(item, DAGNode) and not item.task_group.is_root:
                self._upstream.append(item.task_group)
            else:
    ...
    def set_downstream(
        ...
        for item in self._make_list(other):
            if isinstance(item, DAGNode) and not item.task_group.is_root:
                self._downstream.append(item.task_group)
            else:
                self._downstream.append(item)
